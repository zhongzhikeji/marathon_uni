			...
			<view class="group_item">
				<u-cell-group :border='false'>
					...
					<u-cell icon='https://marathon.zznet.live/file/uploadPath/image/competition/zysx.png' size="large"
						title="注意事项" value="查看" isLink
						@click="$u.route(`/pages/competition/lookout?spuId=${id}`)"></u-cell>
					<u-cell icon='https://marathon.zznet.live/file/uploadPath/image/competition/jsgz.png' size="large"
						title="竞赛规程" value="查看" isLink
						@click="$u.route(`/pages/competition/regulation?spuId=${id}`)"></u-cell>
					<u-cell :border='false'
						icon='https://marathon.zznet.live/file/uploadPath/image/competition/hdzx.png' size="large"
						title="活动资讯" value="查看更多" isLink
				...
				<view>
					<view class="flex  ml10 mr10 pb10">
						<view>
							<u--image v-if="content.relatedArticles" radius='7' :src="content.relatedArticles.picUrl"
								width="66" height="66" @click="click"></u--image>
						</view>
						<view class="ml10">
							...
							<view v-for="(item,index) in infoList" :key='item.id'>
								<view :class="isSelected(item)?'bgColor':'isbgColor'" @click="onTrant(item)"
	                             ...
	                             <view class="mr20" style="color: #EB3D74;">
									 <view v-show="content.skuList[currentIndex].minAge >= item.age || content.skuList[currentIndex].maxAge <=item.age">年龄不符合报名条件</view>
	...
	export default {
		...
		methods: {
			...
			onTrant(item) {
				...
				if(this.content.skuList[this.currentIndex].minAge >= item.age || this.content.skuList[this.currentIndex].maxAge <= item.age ){
